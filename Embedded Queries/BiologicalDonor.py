...
def child_input(procedure):
    ...
    names='''select bd1_first_name,bd1_last_name,bd2_first_name,bd2_last_name from bd_view;'''
    mycursor.execute(names)
    fname1,lname1,fname2,lname2=mycursor.fetchone()
...
def child_status():
    ...
    child_v_status='''select first_name,last_name,date_of_birth,sex,religion,blood_group,skin_color,status,medical_condition,name, house_no, locality, pin_code, state, contact_details from own_child_view_bd;'''
...
def make_donation():
    print("1. Make Donation to random welfare home \n 2. Make donation to select welfare home")
    choice=int(input())
    if(choice==1):
        print()
    elif(choice==2):
        ...
        print("Incorrect/Invalid Option !!!!")
    return
...
def personal_details():
    ...
    personal_details='''select * from bd_view;'''
    mycursor.execute(personal_details)
    for personal_result in mycursor.fetchall():
        id,fname1,lname1,fname2,lname2,dob1,dob2,home,city,pin,state,phone=personal_result
        print("ID:",id)
        print("Parent 1:", fname1,lname1,"Date-of-Birth:",dob1)
        print("Parent 2:", fname2,lname2,"Date-of-Birth:",dob2)
        print("Address:",home,city,pin,state)
        print("Contact:",phone)
    return

def personal_details_temp():
    print("Personal Details")
    personal_details='''select * from temp_bd_view1;'''
    mycursor.execute(personal_details)
    for personal_result in mycursor.fetchall():
        id,fname1,lname1,fname2,lname2,dob1,dob2,home,city,pin,state,phone=personal_result
        print("ID:",id)
...
def edit_details():
    ...
    print('''
        1. First name of Parent 1
        2. Last name of Parent 1
        3. First name of Parent 2
        4. Last name of Parent 2
        5. Date of Birth of Parent 1
        6. Date of Birth of Parent 2
        7. House-No.
        8. Locallity
        9. Pin Code
        10. State
    ...
    new_value=input()
    if(fied_no==1):
        edit_details='''update bd_view set bd1_first_name= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==2):
        edit_details='''update bd_view set bd1_last_name= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==3):
        edit_details='''update bd_view set bd2_first_name= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==4):
        edit_details='''update bd_view set bd2_last_name= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==5):
        edit_details='''update bd_view set bd1_dob= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==6):
        edit_details='''update bd_view set bd2_dob= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==7):
        edit_details='''update bd_view set house_no= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==8):
        edit_details='''update bd_view set locality= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==9):
        edit_details='''update bd_view set pin_code= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==10):
        edit_details='''update bd_view set state= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==11):
        edit_details='''update bd_view set contact_details= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    else:
...
def edit_details_temp():
    ...
    new_value=input()
    if(fied_no==1):
        edit_details='''update temp_bd_view1 set bd1_first_name= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==2):
        edit_details='''update temp_bd_view1 set bd1_last_name= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==3):
        edit_details='''update temp_bd_view1 set bd2_first_name= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==4):
        edit_details='''update temp_bd_view1 set bd2_last_name= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==5):
        edit_details='''update temp_bd_view1 set bd1_dob= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==6):
        edit_details='''update temp_bd_view1 set bd2_dob= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==7):
        edit_details='''update temp_bd_view1 set house_no= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==8):
        edit_details='''update temp_bd_view1 set locality= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==9):
        edit_details='''update temp_bd_view1 set pin_code= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==10):
        edit_details='''update temp_bd_view1 set state= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    elif(fied_no==11):
        edit_details='''update temp_bd_view1 set contact_details= %s where cr_ID = SUBSTRING_INDEX(USER(), '@', 1);'''
    else:
        print("Enter a valid field")
        return
    mycursor.execute(edit_details,(new_value,))
    database.commit()
    personal_details_temp()
    return
def check_verified():
    try:
        check_verified ='''select * from bd_view;'''
...
def print_verified():
    if(check_verified()):
        print("Congratulations!!!\n You have been verified as a Child Renouncer")
    else:
